...
- 当前 `VecWorkflowCoreEnv` 的 `reward_positive_offset=5.0` 是有效 open risk：它按 step 累加，导致未完成但拖到更长 horizon 的策略可能比更快完成 workflow 的策略得到更高 `total_reward`。v39/v41 full-pool 中 DT/cache 的 reward 最高，但 `successful_episode_rate=0.65`、`workflow_continuity_rate=0.583014`、`handoff_failure_rate=0.208333`，而 SA 的对应值是 `1.0`、`0.970369`、`0.0`。因此当前 reward ranking 与 workflow completion/continuity 存在目标不一致风险；若继续追求总 reward 高于 DT/cache，必须先冻结 time-normalized 或 completion-constrained reporting protocol，不能把利用 positive-offset 的延迟行为包装成算法进步。
