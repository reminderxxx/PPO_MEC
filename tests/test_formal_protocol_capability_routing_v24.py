...
def test_capability_registry_is_explicit_active_unique_and_fail_closed() -> None:
    ...
    assert matrix["active_execution_protocol_version"] == "2.9.0"
    assert ACTIVE_EXECUTION_PROTOCOL_VERSION == "2.9.0"
    ...
    assert not get_protocol_capabilities("2.7.0").live_execution_allowed
    assert get_protocol_capabilities("2.8.0").cell_artifact_publication_required
    assert not get_protocol_capabilities("2.8.0").live_execution_allowed
    assert get_protocol_capabilities("2.9.0").cell_artifact_publication_required
    assert get_protocol_capabilities("2.9.0").live_execution_allowed
    ...
    with pytest.raises(FormalProtocolCapabilityError, match="unregistered"):
        get_protocol_capabilities("3.0.0")
