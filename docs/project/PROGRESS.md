...
- commit-bound runner 保留显式 venv 可执行路径，不展开到系统 Python，确保临时依赖覆盖层确实用于测试且不进入正式命令。
- 补充 validation receipt 同时验证快照/验收 runner 的模块导入；发现并修复快照脚本 sibling import 仅支持直接执行的问题。
