...
2026-07-13 已接入 v8-current support suite 入口和 v9 Pareto-safe 候选路径：`scripts/run_strict_full_v8_support_suite.py` 负责补齐 prediction/system/scalability/guard attribution，`top_journal_mechanism_v9_pareto_safe` 负责在 dev / future-validation 上把 handoff failure 与 backhaul 纳入 checkpoint ranking。2026-07-16 进一步新增 `top_journal_mechanism_v10_mappo_rl`，把 MAPPO 的 controller-level CTDE head-credit / entropy-floor 机制迁入 SA-GHMAPPO 候选 profile，同时降低 imitation / mechanism auxiliary 牵引，目标是在 idle/sparse 窗口发挥 PPO 学习能力。当前这些是代码与协议入口，不是新的论文结果；hidden holdout 已 consumed，不能再用于筛选。
