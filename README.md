...
2026-07-13 已接入 v8-current support suite 入口和 v9 Pareto-safe 候选路径：`scripts/run_strict_full_v8_support_suite.py` 负责补齐 prediction/system/scalability/guard attribution，`top_journal_mechanism_v9_pareto_safe` 负责在 dev / future-validation 上把 handoff failure 与 backhaul 纳入 checkpoint ranking。2026-07-16 进一步新增 `top_journal_mechanism_v10_mappo_rl` 与 `top_journal_mechanism_v11_mappo_reward`，把 MAPPO 的 controller-level CTDE head-credit / entropy-floor 机制迁入 SA-GHMAPPO 候选 profile，同时降低 imitation / mechanism auxiliary 牵引，并在 v11 中加入 reward-first checkpoint priority 与 idle/sparse window-context inference gate。v11 full-dev benchmark 已在 frozen dev plan 上让 SA-GHMAPPO total reward 高于全部对照：`79.4944` vs `popularity_cache_heuristic=79.46875`，artifact 为 `artifacts/experiments/top_journal_mappo_reward_full_dev_v11_20260716/main_results_full_stratified_window_gate_full/main_results_full_stratified_20260716_181112_383674/aggregate_summary.json`。该结果仍是 dev evidence，不是 hidden/future-validation 或 paper-ready 结论；hidden holdout 已 consumed，不能再用于筛选。
